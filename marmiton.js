...
var reg="/"+data.RecetteMarmiton+"(.+)/i" ; var rgxp = eval(reg) ; var temp = ClaraIA.reco.match(rgxp) ; console.log(temp)
...
request({'uri':urlmarmiton }, function (error, response, html) {
	...
	urlmarmiton="https://www.marmiton.org/recettes"+listRecetteUrl[0]
		...
		request({'uri':urlmarmiton, 'headers':{'Accept-Charset': 'utf-8'} }, function (error, response, html) {
			...
			ClaraAskMe('veux-tu les ingrédients ou la recette',function(result){
				console.log("retour askme me     :          "+result)
				if(result.search('ingrédients')>-1){ClaraIASpeech(JSON.stringify(listRecetteIngredients));return}
				if(result.search('recette')>-1){ClaraIASpeech(JSON.stringify(listRecettePrepraration));return}	
				ClaraIASpeech('je sors');return
			})
